...
def randomized_response(value, probability):
    # Flip a coin with probability
    if np.random.random() < probability:
        return value
    else:
        # Return a random alternative
        return np.random.choice([0, 1], size=value.shape, p=[0.5, 0.5])

class PeerNode:
    def __init__(self, host, port, key, probability=0.5):
        self.host = host
        self.port = port
        self.peers = []
        self.key = key
        ...
        if os.path.isfile(file_path):
            ...
            with open(file_path, 'rb') as f:
                while (chunk := f.read(4096)):
                    noisy_chunk = randomized_response(chunk, self.probability)
