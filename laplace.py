...
class PeerNode:
        ...
        if os.path.isfile(file_path):
            ...
            with open(file_path, 'rb') as f:
                while (chunk := f.read(4096)):
                    ...
                    print(f"[+] Sending encrypted file chunk")
                    for peer in self.peers:
                        try:
    ...
    def connect_to_tracker(self, tracker_host, tracker_port):
        try:
            ...
            register_message = f"REGISTER {self.host}:{self.port}\n"
            tracker_socket.send(register_message.encode('utf-8'))

            # Request peer list
            tracker_socket.send("GET_PEERS\n".encode('utf-8'))

            peer_list = tracker_socket.recv(1024).decode('utf-8').strip().split(",")
...
if __name__ == "__main__":
    ...
    # Start multiple peer nodes on different ports
    ...
    # Allow time for all nodes to start up and open ports
    time.sleep(2)

    # Simulate one node connecting to a tracker
    tracker_host = '127.0.0.1'
    tracker_port = 8000

    # Only the first node connects to the tracker in this example
