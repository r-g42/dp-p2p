...
def exponential_mechanism(value, sensitivity, epsilon):
    noise = np.random.exponential(scale=sensitivity / epsilon)
    return value + noise
        ...
        if os.path.isfile(file_path):
            ...
            with open(file_path, 'rb') as f:
                while (chunk := f.read(4096)):
                    noisy_chunk = exponential_mechanism(chunk, sensitivity=1.0, epsilon=self.epsilon)
